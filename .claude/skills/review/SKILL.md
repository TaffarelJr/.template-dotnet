...
name: review
description: >-
  Reviews the current change from four independent angles at once - security,
  performance, repo conventions, and documentation accuracy - then merges the
  results into one ranked list. Use before opening a pull request, or when
  asked to review, check, or critique changed code.
when_to_use: >-
...
Run four reviewers in parallel, then synthesise. Do not review the code
...
Dispatch all four in **one message** so they run concurrently:
...
3. Documentation that is now wrong — a reader will trust it and be misled.
4. Will cost the next reader time.
5. Convention drift, or documentation that is merely missing.
